(function () {
  ...
  document.querySelectorAll(".bd-tab").forEach((btn) => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".bd-panel").forEach((p) => (p.style.display = "none"));
      document.getElementById("bd-" + btn.dataset.tab).style.display = "block";
  ...
  document.addEventListener("beerdie:show", () => {
    loadLeaderboard();
    loadPlayers();
  });
  ...
  async function loadPlayers() {
    const res = await fetch("/api/beerdie/players");
  ...
  async function loadLeaderboard() {
    const res = await fetch("/api/beerdie/rankings");
  ...
  document.getElementById("bd-submit-game").addEventListener("click", async () => {
    ...
    const res = await fetch("/api/beerdie/games", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
